...
describe('calcGrade', () => {
    it('returns max grade on perfect score', () => {
        expect(calcGrade(10, 10, DEFAULT.maxGrade, DEFAULT.passingGrade, DEFAULT.passingPct)).toBe(7)
    })

    it('returns 1 on zero score', () => {
        expect(calcGrade(0, 10, DEFAULT.maxGrade, DEFAULT.passingGrade, DEFAULT.passingPct)).toBe(1)
    })

    it('returns exactly the passing grade at the threshold', () => {
        expect(calcGrade(6, 10, DEFAULT.maxGrade, DEFAULT.passingGrade, DEFAULT.passingPct)).toBe(4)
    })
    ...
    it('interpolates below threshold correctly', () => {
        ...
        expect(calcGrade(3, 10, DEFAULT.maxGrade, DEFAULT.passingGrade, DEFAULT.passingPct)).toBe(2.5)
    })

    it('interpolates above threshold correctly', () => {
        // 90% → 4 + (0.3/0.4)*3 = 4 + 2.25 = 6.25 → rounds to 6.3
        expect(calcGrade(9, 10, DEFAULT.maxGrade, DEFAULT.passingGrade, DEFAULT.passingPct)).toBe(6.3)
    })

    it('never exceeds maxGrade', () => {
        expect(calcGrade(100, 100, DEFAULT.maxGrade, DEFAULT.passingGrade, DEFAULT.passingPct)).toBe(7)
    })

    it('never returns less than 1', () => {
        expect(calcGrade(0, 100, DEFAULT.maxGrade, DEFAULT.passingGrade, DEFAULT.passingPct)).toBe(1)
    })
