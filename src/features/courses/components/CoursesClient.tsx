        ...
        <>
                ...
                {courses.length === 0 ? (
                    <Card className="flex flex-col items-center justify-center border-dashed py-24">
                        ...
                        <p className="text-mute mt-1 text-sm">
                            Crea la primera {courseLabel.toLowerCase()} para asignar profesores y alumnos.
                        </p>
                        {canMutate && (
                        ...
                        <div className="border-border flex items-center justify-between border-b px-6 py-4">
                            ...
                            {canMutate && (
                                <Button variant="primary" size="sm" onClick={openCreate}>
                                    <Plus size={14} className="mr-1" /> Nueva {courseLabel.toLowerCase()}
                                </Button>
                            )}
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead className="bg-paper border-border border-b">
                                    <tr>
                                        <th className="text-mute px-6 py-3 text-left font-mono text-[10px] font-bold tracking-[0.1em] uppercase">Materia</th>
                                        <th className="text-mute px-6 py-3 text-left font-mono text-[10px] font-bold tracking-[0.1em] uppercase">Programa</th>
                                        <th className="text-mute px-6 py-3 text-left font-mono text-[10px] font-bold tracking-[0.1em] uppercase">Período</th>
                                        <th className="text-mute px-6 py-3 text-left font-mono text-[10px] font-bold tracking-[0.1em] uppercase">Profesor(es)</th>
                                        <th className="text-mute px-6 py-3 text-center font-mono text-[10px] font-bold tracking-[0.1em] uppercase">Estudiantes</th>
                                        <th className="text-mute px-6 py-3 text-center font-mono text-[10px] font-bold tracking-[0.1em] uppercase">Exámenes</th>
                                    ...
                                    {courses.map(c => (
                                        <tr key={c.id} className="hover:bg-paper-warm/30 transition-colors">
                                            <td className="px-6 py-4">
                                                ...
                                                {c.code && <p className="text-xs text-mute font-mono">{c.code}</p>}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-center font-bold text-ink">{c._count.exams}</td>
                                            <td className="px-6 py-4 text-right">
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        <DropdownMenuItem onClick={() => router.push(`/${slug}/courses/${c.id}`)} className="cursor-pointer gap-2 py-2">
                                                            <BookOpen size={14} /> Ver detalle
                                                        </DropdownMenuItem>
                                                        {canMutate && (
                                                            <>
                                                                <DropdownMenuItem onClick={() => openEdit(c)} className="cursor-pointer gap-2 py-2">
                                                                    <Edit2 size={14} /> Editar
                                                                </DropdownMenuItem>
                                                                <DropdownMenuItem onClick={() => openDelete(c)} className="text-destructive cursor-pointer gap-2 py-2">
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogContent className="border-border rounded-[22px] shadow-2xl max-w-2xl">
                    ...
                    <DialogFooter>
                        <Button variant="ghost" onClick={() => setIsOpen(false)} disabled={isPending}>Cancelar</Button>
            ...
            <AlertDialog open={isDelOpen} onOpenChange={setIsDelOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-destructive">Eliminar {courseLabel.toLowerCase()}</AlertDialogTitle>
                        <AlertDialogDescription>¿Estás seguro de eliminar <strong>{toDelete?.name}</strong>?</AlertDialogDescription>
                    </AlertDialogHeader>
