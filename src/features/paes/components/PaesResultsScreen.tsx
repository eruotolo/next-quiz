...
import type { PaesExam, PaesQuestion, PaesQuestionResult, PaesResult } from '@/features/paes/types/paes.types';
    ...
    return (
        <div className="min-h-screen bg-paper">
            ...
            <div className="mx-auto max-w-3xl px-4 py-10">
                ...
                {/* Summary card */}
                <div className="mb-6 overflow-hidden rounded-[18px] border border-border bg-white shadow-sm">
                    ...
                    <div className="grid grid-cols-4 divide-x divide-border">
                        {[
                            ...
                            { label: 'Incorrectas', value: results.incorrect, color: 'text-destructive' },
                            { label: 'Sin responder', value: results.unanswered, color: 'text-mute' },
                            { label: 'Porcentaje', value: `${results.percent}%`, color: 'text-ink' },
                        ].map((stat) => (
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex flex-col gap-2">
                            {results.byEje.map((eje) => {
                                const ejePct = eje.total > 0 ? Math.round((eje.correct / eje.total) * 100) : 0;
                                return (
                                    <div
                                        key={eje.eje}
                                        className="flex items-center gap-4 rounded-[12px] border border-border bg-white px-5 py-4"
                                    >
                ...
                <div className="mb-6">
                    ...
                    <div className="flex flex-col gap-4">
                        {results.perQuestion.map((qr, idx) => {
                            ...
                            return <PaesQuestionReview key={qr.questionId} qr={qr} question={question} idx={idx} />;
                        })}
                    </div>
                </div>
                ...
                <div className="flex flex-col items-center gap-4 rounded-[16px] border border-border bg-white px-8 py-6 text-center">
                    <p className="font-mono text-[11px] uppercase tracking-[0.08em] text-mute">
                        El puntaje estimado es referencial — no constituye un puntaje oficial de la PAES
                    </p>
    ...
    return (
        <div
            className={cn(
                ...
                qr.isCorrect ? 'border-success/30' : isUnanswered ? 'border-border' : 'border-destructive/30',
            )}
        >
            <div
                className={cn(
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-[9px] text-mute">{question.eje}</span>
                </div>
                <div className="flex items-center gap-1.5">
                    {qr.isCorrect ? (
                        <>
                            <CheckCircle2 size={14} className="text-success" />
                            <span className="font-mono text-[11px] font-semibold text-success">Correcta</span>
                        </>
                    ) : isUnanswered ? (
                        <>
                            <Circle size={14} className="text-mute" />
                            <span className="font-mono text-[11px] font-semibold text-mute">Sin responder</span>
                        </>
                    ) : (
                        <>
                            <XCircle size={14} className="text-destructive" />
                            <span className="font-mono text-[11px] font-semibold text-destructive">Incorrecta</span>
                        </>
                    )}
                </div>
            </div>
            ...
            <div className="px-6 py-5">
                <p className="mb-5 text-[15px] font-semibold leading-snug text-ink">{question.statement}</p>
                <div className="flex flex-col gap-2">
...
function PaesOptionReview({
    ...
    return (
        <div className={cn('flex items-center gap-3 rounded-[10px] border px-4 py-3', OPTION_ROW_CLASS[state])}>
            <span
                className={cn(
            ...
            <span className={cn('flex-1 text-[14px]', OPTION_TEXT_CLASS[state])}>{option.text}</span>
