...
describe('evaluateCriterion', () => {
    it('TOTAL_POINTS', () => {
        expect(
            evaluateCriterion({ type: 'TOTAL_POINTS', threshold: 100 }, stats({ totalPoints: 99 })),
        ).toBe(false);
        expect(
            evaluateCriterion(
    ...
    it('LESSONS_COMPLETED', () => {
        expect(
            evaluateCriterion(
                { type: 'LESSONS_COMPLETED', threshold: 5 },
    ...
    it('ASSIGNMENTS_SUBMITTED', () => {
        expect(
            evaluateCriterion(
                { type: 'ASSIGNMENTS_SUBMITTED', threshold: 1 },
        ...
        expect(
            evaluateCriterion(
                { type: 'ASSIGNMENTS_SUBMITTED', threshold: 1 },
                stats({ assignmentsSubmitted: 1 }),
            ),
        ).toBe(true);
    });
    it('EXAMS_PASSED', () => {
        expect(
            evaluateCriterion({ type: 'EXAMS_PASSED', threshold: 3 }, stats({ examsPassed: 2 })),
        ).toBe(false);
        expect(
            evaluateCriterion({ type: 'EXAMS_PASSED', threshold: 3 }, stats({ examsPassed: 3 })),
        ).toBe(true);
    });
    it('FORUM_POSTS', () => {
        expect(
            evaluateCriterion({ type: 'FORUM_POSTS', threshold: 10 }, stats({ forumPosts: 9 })),
        ).toBe(false);
        expect(
            evaluateCriterion({ type: 'FORUM_POSTS', threshold: 10 }, stats({ forumPosts: 10 })),
        ).toBe(true);
    });
    it('LONGEST_STREAK', () => {
        expect(
            evaluateCriterion(
                { type: 'LONGEST_STREAK', threshold: 7 },
                stats({ longestStreak: 6 }),
            ),
        ).toBe(false);
        expect(
            evaluateCriterion(
