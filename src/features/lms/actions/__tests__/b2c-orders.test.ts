...
describe('createLmsCheckoutPreference', () => {
    ...
    it('rechaza payload inválido (Zod)', async () => {
        const res = await createLmsCheckoutPreference('colegio-demo', {
            courseId: 'invalid',
            studentRut: '12.345.678-0', // DV incorrecto
            studentName: 'J',
            studentLastname: 'P',
    ...
    it('rechaza si la institución no existe', async () => {
        prismaInstitutionFindUniqueMock.mockResolvedValueOnce(null);
        const res = await createLmsCheckoutPreference('no-existe', VALID_INPUT);
        expect(res.error).toMatch(/instituci/i);
        expect(res.data).toBeNull();
    });

    it('rechaza si la institución está inactiva', async () => {
        prismaInstitutionFindUniqueMock.mockResolvedValueOnce({
            id: 'inst-1',
            name: 'X',
            active: false,
        });
        const res = await createLmsCheckoutPreference('colegio-demo', VALID_INPUT);
        expect(res.error).toMatch(/instituci/i);
    });
    ...
    it('normaliza RUT y lowercase email al crear la orden', async () => {
        await createLmsCheckoutPreference('colegio-demo', VALID_INPUT);
