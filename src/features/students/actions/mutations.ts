...
async function professorHasAccess(
    ...
    const student = await prisma.user.findFirst({
        ...
        select: { group: { select: { professors: { where: { id: professorId }, select: { id: true } } } } },
    });
    return (student?.group?.professors.length ?? 0) > 0;
}

export async function createStudent(slug: string, data: unknown): Promise<ActionResult<{ id: string }>> {
    try {
...
export async function updateStudent(slug: string, id: string, data: unknown): Promise<ActionResult> {
    try {
        ...
        const res = await prisma.user.updateMany({
            where: { id, academicInstitutionId: ctx.institutionId, userRole: { name: USER_ROLE.STUDENT } },
            data: parsed.data,
        });
