...
- **Estética Sobria y Premium (UI):** Siguiendo las instrucciones de diseño del usuario, se evitarán tipografías excesivamente gigantes. Se utilizarán títulos y precios con tamaños de fuente moderados y estilizados (ej. `text-3xl` o `text-4xl` para encabezados principales y precios en tamaño intermedio con peso medium/semibold) para un acabado elegante y equilibrado.
...
└── app/
    ...
    └── (public)/
        ...
        ├── cursos/
        │   └── page.tsx                          ← NUEVO: Vitrina comercial premium que lista todos los cursos públicos de aulika-online y el Pack Completo con títulos y precios moderados.
        └── components/
...
- [ ] **Crear la página pública `/cursos` (`src/app/(public)/cursos/page.tsx`):**
  ...
  - **Especificación de Escala Visual (CRÍTICA):** Los títulos de los cursos y la tipografía de los precios deben mantener dimensiones sobrias y elegantes (evitando clases gigantes como `text-5xl` o `text-6xl`, prefiriendo `text-xl`/`text-2xl` para títulos en tarjetas y pesos medianos para el precio).
- [ ] **Crear `src/features/landing/components/L3PreuPDV.tsx`:**
  Desarrollar una sección premium en la Home que promocione la vitrina e invite a hacer clic en el CTA principal ("Ver catálogo de cursos") para registrar la redirección a `/cursos`. Títulos y precios con escala tipográfica sobria.
